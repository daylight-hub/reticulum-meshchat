...
def install_bridge_on_telephone(telephone, bridge):
    ...
    if original_reconf is not None:
        def reconfigure_transmit_override(*args, **kwargs):
            # Minimal mirror of LXST's own __reconfigure_transmit_pipeline. The ONLY
            # difference is that we rebuild with our WebSocket mic source instead of a
            # LineSource, because a LineSource needs PulseAudio which does not exist in
            # Docker (it raises libpulse.so errors and kills the call). No codec-switch
            # cleverness here - LXST's behaviour is followed exactly.
            ...
            try:
                ...
                if t.transmit_pipeline and t.call_status == Signalling.STATUS_ESTABLISHED:
                    ...
                    # tell the browser the mic frame size for the new profile
                    try:
                        target_ms = getattr(t.transmit_mixer, "target_frame_ms", None) or t.target_frame_time_ms
                        spf = int(round((target_ms / 1000.0) * BRIDGE_SAMPLERATE))
                        if spf > 0:
                            bridge.set_target_frame_samples(spf)
                            RNS.log(f"WebRTC bridge: target frame {target_ms}ms -> {spf} samples", RNS.LOG_NOTICE)
                    except Exception:
                        pass

                    # rebuild with our WebSocket mic source instead of a LineSource
