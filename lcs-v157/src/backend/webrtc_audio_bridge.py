        ...
        def reconfigure_transmit_override(*args, **kwargs):
            ...
            try:
                ...
                if t.transmit_pipeline and t.call_status == Signalling.STATUS_ESTABLISHED:
                    ...
                    # what frame size does the NEW profile want?
                    new_target_ms = t.target_frame_time_ms
                    new_spf = int(round((new_target_ms / 1000.0) * BRIDGE_SAMPLERATE)) if new_target_ms else None

                    # If the transmit_mixer's frame timing is unchanged (e.g. Medium->High,
                    # both 60ms Opus), a full teardown/rebuild is unnecessary and actually
                    # breaks the receive side (it orphans the echo-suppressor reference held
                    # by receive_mixer). In that case only swap the transmit CODEC in place.
                    current_target_ms = getattr(t.transmit_mixer, "target_frame_ms", None)
                    same_framing = (current_target_ms == new_target_ms)

                    if same_framing:
                        # just point the existing transmit pipeline at the new codec
                        try:
                            t.transmit_pipeline.codec = t.transmit_codec
                        except Exception:
                            pass
                        if new_spf:
                            bridge.set_target_frame_samples(new_spf)
                        RNS.log(f"WebRTC bridge: codec swapped in place (framing unchanged, "
                                f"{new_target_ms}ms); receive pipeline left intact", RNS.LOG_NOTICE)
                        return

                    # --- framing actually changed (e.g. Opus<->Codec2): full rebuild ---
                    # stop the old transmit chain
                    try:
                    ...
                    # rebuild mixer for the new target frame time
                    t.transmit_mixer = Mixer(target_frame_ms=t.target_frame_time_ms, gain=t.transmit_gain)

                    # re-wire the echo suppressor reference to the NEW transmit mixer path,
                    # so the receive mixer keeps a valid reference and inbound audio flows.
                    try:
                        es = getattr(t.active_call, "echo_suppressor", None)
                        if es is not None and hasattr(t, "receive_mixer") and t.receive_mixer is not None:
                    ...
                    if new_spf:
                        bridge.set_target_frame_samples(new_spf)
                        RNS.log(f"WebRTC bridge: reconfigured target frame {new_target_ms}ms -> {new_spf} samples", RNS.LOG_NOTICE)
