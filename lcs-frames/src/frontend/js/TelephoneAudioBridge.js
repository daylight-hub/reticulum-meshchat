...
export default class TelephoneAudioBridge {
    ...
    async start() {
        ...
        // We must emit frames whose duration is valid for BOTH Opus and Codec2.
        // Opus only accepts 2.5/5/10/20/40/60 ms frames; 20 ms is the safe common size.
        // At 48000 Hz, 20 ms = exactly 960 samples.
        // ScriptProcessorNode buffer sizes must be powers of two (can't be 960), so we
        // capture at 2048 and re-chunk the stream into exact 960-sample frames.
        const OUTPUT_FRAME_SAMPLES = Math.round(BRIDGE_SAMPLERATE * 0.02); // 960 @ 48k
        ...
        this.processor.onaudioprocess = (e) => {
            ...
            // emit as many full 960-sample frames as we have
            let offset = 0;
            while (merged.length - offset >= OUTPUT_FRAME_SAMPLES) {
                const frame = merged.subarray(offset, offset + OUTPUT_FRAME_SAMPLES);
