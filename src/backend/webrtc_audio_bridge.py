...
def install_bridge_on_telephone(telephone, bridge):
    ...
    if original_prepare is not None:
        def prepare_wrapper(self, *args, **kwargs):
            ...
            if self.audio_output is None:
                self.audio_output = bridge.make_sink()
            return original_prepare(*args, **kwargs)
        setattr(telephone, prepare_name, types.MethodType(prepare_wrapper, telephone))
    else:
    ...
    if original_open is not None:
        def open_wrapper(self, identity, *args, **kwargs):
            ...
            try:
                ...
                if getattr(self, "audio_input", None) is not None:
                    old = self.audio_input
                    sink = getattr(old, "sink", None) or getattr(self, "transmit_mixer", None)
                    filters = getattr(self.active_call, "filters", []) if getattr(self, "active_call", None) else []
                    try:
                        old.stop()
                    except Exception:
                        pass
                    ws_source = bridge.make_source(sink=sink, filters=filters)
                    ws_source.codec = getattr(old, "codec", None)
                    self.audio_input = ws_source
                    bridge.activate()
                    ws_source.start()
                    RNS.log("WebRTC bridge: microphone swapped to WebSocket source", RNS.LOG_DEBUG)
            except Exception as e:
                RNS.log(f"WebRTC bridge: mic swap failed: {e}", RNS.LOG_WARNING)
            return result
        setattr(telephone, open_name, types.MethodType(open_wrapper, telephone))
    else:
